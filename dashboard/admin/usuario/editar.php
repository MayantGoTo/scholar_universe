...
if (isset($_POST['update'])) {
    ...
    $id = $user->id;
    if ($val->isEmpty($nombre) || $val->isEmpty($email) || $val->isEmpty($apellido_paterno)) {
        $error = "Todos los campos son obligatorios";
    } else {
        $bnd = $email == $user->email ? false : $usuario->validaEmail($email);
        if ($bnd) {
            $error = "el correo electrónico ya está registrado";
        } else {
            if ($password == $confirmar_password) {
                if ($usuario->guardaActualiza($id, $nombre, $apellido_paterno, $apellido_materno, $email, $password, "2")) {
            ...
            <form method="POST" action="" enctype="application/json">
                ...
                <div class="mb-3">
                    ...
                    <input type="email" class="form-control" id="email" name="email" placeholder="Email" required
                        maxlength="50" minlength="1" value="<?= $user->email ?>">
                </div>
